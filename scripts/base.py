...
if sys.argv[2] == 'europe':
    ...
    M = int(sys.argv[3])
    stan_data, regions, start_date, geocode = get_data_county(M, data_dir, interpolate=False, filter_data=True)
    ...
    M = int(sys.argv[3])
    stan_data, regions, start_date, geocode = get_data_state(M, data_dir, interpolate=False, filter_data=True)
